...
create_new_session() {
    ...
    "$CURRENT_DIR/tmux-sessionizer.sh" new
}
    ...
    while true; do
        ...
        selected_session=$(get_sessions_list | \
            fzf \
                ...
                --header-lines=0 \
                --preview="$CURRENT_DIR/tmux-session-manager.sh preview {1}" \
                ...
                --info=inline \
                --bind="ctrl-a:execute($CURRENT_DIR/tmux-session-manager.sh new-session)+reload($CURRENT_DIR/tmux-session-manager.sh list)" \
                --bind="ctrl-r:execute($CURRENT_DIR/tmux-session-manager.sh rename {1})+reload($CURRENT_DIR/tmux-session-manager.sh list)" \
                --bind="ctrl-x:execute($CURRENT_DIR/tmux-session-manager.sh kill {1})+reload($CURRENT_DIR/tmux-session-manager.sh list)" \
                --bind="?:execute($CURRENT_DIR/tmux-session-manager.sh help)" \
                --expect=ctrl-c,esc,q
        )
